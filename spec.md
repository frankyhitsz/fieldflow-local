...
当前发布目标为 v0.5.7：统一应急风险时间线，细化 PlanApplicability，补全 RiskComparison 与容量 Artifact 的消费门禁，并修正不会破坏既有路线的数据变更语义。
...
- `PlanVersion` 的冻结 payload 不保存运行时 `active`、`coverage_status` 或名称变化；Schema v20 的 `plan_applicability` 独立投影路线可执行、覆盖完整、规划/指标/商业数据是否当前、再优化机会和无效 assignment，名称、备注和标签由 `plan_metadata` 覆盖。
...
- pending 分配保存 assignment-feasibility fingerprint；技能、位置、时长、时间窗、班次、锁定或行程模型变化后，旧分配不能继续开工。优先级、VIP、drop penalty 等目标字段会让方案提示指标过期，但不伪装成分配约束；姓名、备注等元数据不误伤执行。普通新工单保留既有路线并标为部分覆盖，新增技师标为再优化机会，删除未分配需求不撤销路线。
- 网页写入 WorkOrder、Technician、Lock 和 Reset 时携带 `If-Match: Dnnn`。服务端还保留事务内修订比较，过期客户端意图返回结构化 409。
...
- 风险默认遵循正式方案开始时刻。replan 路线从发布时 route entry 开始。随机量按 `(seed, trial, event_type, entity_id)` 派生；计划无关的场景集保存突发事件时间、位置、时长、技能和 SLA。每个 trial 只有一份可变路线状态；`BETWEEN_VISITS_ONLY` 不在行驶途中改道、不抢占服务，候选状态直接续跑正式路线。响应人按 `EARLIEST_FEASIBLE_COMPLETION` 选择，两项政策同时进入请求、结果、场景集和哈希。`published_commitment_sla_rate` 只度量已发布承诺，`all_demand_sla_rate` 把应急需求纳入分母，并另列应急完成、准时和未服务概率。
- 配对比较必须复用同一场景集哈希，并持久保存发布承诺 SLA、全需求 SLA、紧急完成、紧急准时、加班、未服务和致损的配对差值、95% 区间和 win/tie/loss。它绑定两个 A 的清单以及 trial、scenario-set Artifact 哈希；幂等预检发生在子分析之前，GET 和重放都重新验证全部依赖。任一证明失效时整个业务 `result` 及兼容数值投影均为空。
...
- Schema v10 把技师浮点成本保值迁移为整数分；v11 增加命令发布键；v12 增加方案适用性投影；v13–v17 增加经营分析运行、恢复、重试和 Artifact；v18 增加不可降级证明要求、风险比较、方案元数据和损坏隔离；v19 增加 A 关系型状态机及传递信任；v20 增加多维 PlanApplicability。
...
- Schema v1–v19 均迁移到 v20 并通过完整性、Artifact 保全、关系型终态、适用性和外键检查。
