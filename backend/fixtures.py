...
def _technicians() -> list[Technician]:
    return [
        Technician(
            id="TECH-01",
            name="林乔",
            skills=[Skill.electrical, Skill.network],
            shift_start=480,
            shift_end=927,
            start_location=Point(x=48, y=52),
            overtime_limit=60,
            color="#315c4b",
        ),
        Technician(
            id="TECH-02",
            name="周岚",
            skills=[Skill.hvac, Skill.electrical],
            shift_start=510,
            shift_end=850,
            start_location=Point(x=48, y=52),
            overtime_limit=45,
        ...
        Technician(
            id="TECH-03",
            name="沈川",
            skills=[Skill.network, Skill.hvac],
            shift_start=480,
            shift_end=930,
            start_location=Point(x=48, y=52),
            overtime_limit=60,
    ...
    for i in range(24):
        ...
        orders.append(
            WorkOrder(
                ...
                location=Point(
                    x=max(5, min(95, ax + rng.randint(-10, 10))), y=max(5, min(95, ay + rng.randint(-10, 10)))
                ),
                ...
                priority=priority,
                drop_penalty={Priority.urgent: 8500, Priority.high: 4800, Priority.normal: 2500, Priority.low: 1400}[
                    priority
                ],
                vip=vip,
...
def _strategy_fixture(scenario_id: str, technician_count: int, order_count: int, seed: int) -> ScheduleScenario:
    ...
    for index in range(technician_count):
        ...
        technicians.append(
            Technician(
                id=f"TECH-LAB-{index + 1:02d}",
                name=f"实验技师{index + 1:02d}",
                skills=tech_skills,
                shift_start=480 + (index % 3) * 20,
                shift_end=960 - (index % 4) * 15,
                start_location=Point(x=48 + (index % 2) * 4, y=48 + ((index // 2) % 2) * 4),
                overtime_limit=[20, 40, 60][index % 3],
                color=colors[index],
            )
        )
    ...
    for index in range(order_count):
        ...
        orders.append(
            WorkOrder(
                id=f"WO-LAB-{index + 1:03d}",
                customer_name=f"{CUSTOMERS[index % len(CUSTOMERS)]}·{index + 1:02d}",
                title=rng.choice(TITLES[skill]),
                ...
                window_start=start,
                window_end=start + width,
                sla_deadline=start + min(width, [55, 75, 100][index % 3]),
                priority=Priority.urgent if vip else priority,
                drop_penalty=10000
                if vip
                else {Priority.urgent: 8500, Priority.high: 4800, Priority.normal: 2500, Priority.low: 1400}[priority],
                vip=vip,
                note="已向客户承诺到场" if vip else "",
            )
        )
    return ScheduleScenario(
        id=scenario_id,
...
def all_fixtures() -> dict[str, ScheduleScenario]:
    ...
    skill_orders.append(
        WorkOrder(
            ...
            customer_name="精密实验室",
            title="冷媒高压系统检修",
            required_skills=[Skill.hvac, Skill.network],
            location=Point(x=88, y=12),
            service_duration=90,
            window_start=600,
            window_end=720,
            sla_deadline=690,
    ...
    return {
        s.id: s for s in [main, skill_scenario, window_scenario, emergency_scenario, strategy_medium, strategy_stress]
    }
