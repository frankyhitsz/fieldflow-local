...
The application factory delays Store initialization and worker creation until FastAPI lifespan startup. Only that application-startup Store performs restart reconciliation of interrupted jobs, analyses, experiments, schedule runs and commands. Solver children and artifact-maintenance commands open a non-recovering Store view; opening a second database handle must not be interpreted as an application restart or revoke a live worker lease. The current module still uses a process-local Store binding, so two app instances should not be served concurrently in one process.
