...
SQLite schema v21 uses foreign keys for scenario-owned data, including a composite scenario/Plan reference for applicability, and constrains applicability enums and JSON arrays in the database. It enforces one parent for every schedule artifact, limits each schedule run to one candidate, separates plan applicability and display metadata, and stores execution events, decision analyses, analysis artifacts and paired risk comparisons. `(logical_analysis_id, attempt_number)`, public plan numbers, execution actions, and comparison idempotency keys are database-unique. Required attestation and terminal-state triggers close legacy downgrade and RUNNING rollback paths. Manual reassignment preallocates a stable schedule-run identity so restart recovery cannot duplicate the lock, revision, run or plan version; post-lock context conflicts become terminal commands. Connections use WAL and a busy timeout. Migrations create a timestamped backup, preserve valid artifacts, conservatively translate legacy applicability, and copy rejected records into the quarantine ledger. Malformed Plan, applicability, execution, or A rows are rejected or isolated with a stable integrity error rather than silently projected into business output.
