...
type Skill = Technician['skills'][number]
const skillEntries = Object.entries(skillLabel) as [Skill, string][]
...
export function TechniciansView({ scenario, schedule, onEdit, onAdd }: { scenario: Scenario; schedule?: Schedule; onEdit: (tech: Technician) => void; onAdd: () => void }) {
  const coverage = (Object.keys(skillLabel) as Skill[]).map(skill => ({ skill, count: scenario.technicians.filter(t => t.skills.includes(skill)).length }))
  return <section className="page-view">
...
export function ReviewView({ scenarioId, planVersionId, schedule, baseline }: { scenarioId: string; planVersionId?: string; schedule?: Schedule; baseline?: Schedule }) {
  ...
  const [emergencyLocationPolicy, setEmergencyLocationPolicy] = useState<RiskSimulation['emergency_location_policy']>('ALL_FROZEN_LOCATIONS_AS_SPATIAL_PROXY')
  ...
  return <section className="page-view">
    ...
    <div className="decision-head"><div><span className="eyebrow">DECISION SUPPORT</span><h2>经营决策测算</h2><p>分析绑定 V{String(cost?.plan_number ?? risk?.plan_number ?? schedule.version).padStart(3, '0')} 的冻结方案、旅行模型、政策与代码来源。进入页面只读取已有 A；点击生成才会新增记录，不会生成 D 或 V。</p></div><div className="decision-actions"><label>成本/容量周期（工作日）<input aria-label="分析工作日" type="number" min="1" max={publicationRemaining ? 1 : 365} disabled={publicationRemaining} value={effectiveHorizonDays} onChange={event => setHorizonDays(Math.max(1, Math.min(365, Number(event.target.value) || 1)))} /><small>{publicationRemaining ? '剩余计划固定为当日一次性口径' : '完整计划可按工作日外推'}</small></label><label>突发位置样本<select aria-label="突发位置样本" value={emergencyLocationPolicy} onChange={event => { setEmergencyLocationPolicy(event.target.value as RiskSimulation['emergency_location_policy']); setRisk(undefined); setAnalysisNumbers(current => ({ ...current, risk: undefined })) }}><option value="ALL_FROZEN_LOCATIONS_AS_SPATIAL_PROXY">全部冻结地点作为区域代理</option><option value="ACTIVE_DEMAND_LOCATIONS">仅当前待服务需求地点</option><option value="UNIFORM_SERVICE_AREA">服务区域内均匀采样</option></select></label><button disabled={!planVersionId || !planTrusted || loadingDecision} onClick={runCostAndRisk}>生成成本与风险分析</button><label>容量参照<select aria-label="容量分析参照" value={capacityMode} onChange={event => { setCapacityMode(event.target.value as typeof capacityMode); setCapacity(undefined); setAnalysisNumbers(current => ({ ...current, capacity: undefined })) }}><option value="SELECTED_PLAN_DELTA">相对当前 V</option><option value="CONTROLLED_REOPTIMIZATION">相对同算法重算基线</option></select></label><button disabled={!planVersionId || !planTrusted || loadingDecision} onClick={runCapacity}>测算六种容量方案</button></div></div>
    ...
    {(cost || risk) && <div className="decision-summary">
      ...
      <article><small>全需求风险 SLA</small><b>{risk ? pct(risk.all_demand_sla_rate) : '—'}</b><p>{risk ? risk.emergency_event_count ? `紧急单完成 ${pct(risk.emergency_completion_rate ?? 0)} · 按时 ${pct(risk.emergency_on_time_rate ?? 0)} · 样本 ${risk.emergency_event_count}` : '紧急指标不适用：本次模拟未发生紧急事件' : '原计划与实际发生的紧急需求使用同一分母'}</p></article>
      <article><small>{remainingScope ? '剩余发布工单迟到 P95' : '发布工单迟到 P95'}</small><b>{risk ? `${risk.published_work_total_late_minutes_p95} 分钟` : '—'}</b><p>P50 {risk?.published_work_total_late_minutes_p50 ?? '—'} · P90 {risk?.published_work_total_late_minutes_p90 ?? '—'} 分钟</p></article>
      <article><small>全需求总迟到 P95</small><b>{risk ? `${risk.all_demand_total_late_minutes_p95} 分钟` : '—'}</b><p>包含实际发生且完成的紧急需求；P50 {risk?.all_demand_total_late_minutes_p50 ?? '—'} 分钟</p></article>
      <article><small>紧急单迟到</small><b>{risk?.emergency_late_minutes_mean == null ? '不适用' : `${risk.emergency_late_minutes_mean} 分钟`}</b><p>{risk ? `仅统计完成样本 ${risk.emergency_completed_sample_count}/${risk.emergency_metric_sample_count}；P90 ${risk.emergency_late_minutes_p90 ?? '—'} 分钟` : '—'}</p></article>
      <article><small>新增业务损害概率</small><b>{risk ? pct(risk.additional_disruption_probability) : '—'}</b><p>{risk ? `突发事件发生 ${pct(risk.emergency_event_probability)} · 实际致损 ${pct(risk.emergency_caused_failure_probability)} · 条件致损 ${risk.emergency_failure_given_event_probability == null ? '不适用' : pct(risk.emergency_failure_given_event_probability)}` : '—'}</p></article>
