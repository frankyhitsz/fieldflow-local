    ...
    <div className="version-list">
      {visible.map(item => {
        ...
        return <article key={item.id} className={`version-row ${item.active ? 'current' : ''}`}>
          <button className="version-open" onClick={() => onOpen(item)} aria-label={`打开 V${String(item.number).padStart(3, '0')}`}><div className="version-mark"><b>V{String(item.number).padStart(3, '0')}</b><span>{item.active ? '当前执行方案' : kindLabel[schedule.kind]}</span></div></button>
...
export function WorkOrderEditor({ initial, emergencyPreset, onClose, onSave, onDelete }: { initial?: WorkOrder; emergencyPreset?: boolean; onClose: () => void; onSave: (order: WorkOrder, replan: boolean) => Promise<void>; onDelete?: (order: WorkOrder) => Promise<void> }) {
  ...
  return <div className="modal-backdrop"><section className="editor-modal" role="dialog" aria-modal="true" aria-labelledby="work-order-editor-title">
    ...
    <div className="form-grid"><label className="span-2">客户名称<input disabled={executionLocked} value={order.customer_name} onChange={e => patch('customer_name', e.target.value)} placeholder="例如：衡安数据中心" /></label><label className="span-2">任务内容<input disabled={executionLocked} value={order.title} onChange={e => patch('title', e.target.value)} placeholder="例如：核心机房断电告警" /></label>
      ...
      <label>优先级<select disabled={executionLocked} value={order.priority} onChange={e => patch('priority', e.target.value as WorkOrder['priority'])}><option value="urgent">紧急</option><option value="high">高</option><option value="normal">普通</option><option value="low">低</option></select></label><label>状态<select value={order.status} onChange={e => patch('status', e.target.value as WorkOrder['status'])}>{initial?.status === 'pending' || !initial ? <option value="pending">待处理</option> : null}{initial?.status !== 'completed' && <option value="started">已开始</option>}<option value="completed">已完成</option></select></label>
